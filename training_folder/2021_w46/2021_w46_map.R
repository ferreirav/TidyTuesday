...
house_iso <- ors_isochrones(locations = coordinates, profile = "foot-walking",
                            range = 6000, interval = 600, output = "sf")
...
# Alternative collor pallet is called "magma"
...
walking_map <- ggplot() +
  ...
  coord_sf(xlim = custom_fulham[1,],
           ...
           expand = FALSE) +
  with_outer_glow(annotate(geom = "text", label = "Walking the streets of Fulham",
                           x = -0.20, y = 51.557, size = 7.5, hjust = 0.5, colour = colpal[10],
                           family = "mono"),
                  colour = colpal[4], sigma = 10, expand = 10) +
  with_outer_glow(annotate(geom = "text", label = "How far can one walk from Fulham in 100 minutes?",
                           x = -0.20, y = 51.550, size = 4, hjust = 0.5, colour = colpal[10], family = "mono"),
  ...
  with_outer_glow(annotate(geom = "text", label = "@Vitor Ferreira | source = © openrouteservice.org by HeiGIT | Map data © OpenStreetMap contributors",
                           x = -0.20, y = 51.419, size = 2, hjust = 0.5, colour = colpal[10], family = "mono"),
...
ggsave("./training_folder/2021_w46/fulham_walking_map.png", walking_map, width = 12, height = 6.1)
