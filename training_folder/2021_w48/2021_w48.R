...
# Data on Doctor Who Series

# Code Sourced from: https://github.com/tashapiro/TidyTuesday/blob/master/2021/W48/doctor_who.R
...
# join averages by doctor to the main df
...
p <- ggplot(df, aes(x=reorder(doctor, avg_doc_rating), y=rating)) +
  ...
  geom_point(aes(x=doctor, y=avg_doc_rating), color="white",size=12)+
  geom_image(aes(x=doctor, y=avg_doc_rating,image=image),asp=1.7)+
  ...
  coord_flip()+
  labs(
    title='DOCTOR WHO - BEST RATING ACCROS SEASONS',
    subtitle = "Ratings Per Doctor Who Episode, Scaled 1-100",
    x="DOCTOR",
    y="RATINGS",
    caption="Data from datardis | Original Chart @tanya_shapiro | Replicate by Vitor Ferreira"
  )+
...
p <- p + geom_curve(data = arrows%>%filter(curve=="up"), aes(x = x1, y = y1, xend = x2, yend = y2),
...
ggsave("./training_folder/2021_w48/doctor_who.png", width = 12, height = 8)
